...
// ============================================================
// DELETE: Desativar (soft) ou Apagar definitivamente (hard)
// Query params: ?userId=xxx  ou  ?userId=xxx&hard=true
// ============================================================
async function deleteUser(req, res) {
  ...
  if (hard) {
    ...
    if (!delProfileRes.ok) {
      ...
      return res.status(500).json({ error: 'Erro ao remover profile', detail: errText.substring(0, 200) });
    }
    ...
    if (!delAuthRes.ok) {
      ...
      return res.status(500).json({ error: 'Profile removido, mas falhou ao remover do Auth', detail: errText.substring(0, 200) });
    }

    return res.status(200).json({ success: true, message: 'Usuário excluído definitivamente' });
  }
